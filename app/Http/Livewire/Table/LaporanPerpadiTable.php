...
class LaporanPerpadiTable extends LivewireDatatable
{
    public function builder()
    {

        return Laporan::where('sumber', 'perpadi')->orderBy('created_at', 'desc');
    }
            ...
            Column::callback(['id', 'status'], function ($id, $status) {
                if ($status == 'pending') {
                    return '<div class="flex">
                                <x-button onclick="confirm(\'Are you sure you want to approve this item?\') || event.stopImmediatePropagation()" wire:click="approve(' . $id . ')" class="bg-green-500 hover:bg-green-700 text-yellow-500 font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">Approve</x-button>
                                <x-button onclick="confirm(\'Are you sure you want to reject this item?\') || event.stopImmediatePropagation()" wire:click="reject(' . $id . ')" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline ml-2">Reject</x-button>
                            </div>';
